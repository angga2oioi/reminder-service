...
amqp.connect(process.env.AMQP_HOST, (err, conn) => {
  ...
  conn.createChannel((error1, ch) => {
    ...
    channel.assertQueue(REMOVE_REMINDER_BY_USER_ID_MQ_QUEUE, {
      durable: false,
    });
  });
});
...
exports.submitRemoveReminderByUserId = (payload = {}) => {
  const msg = JSON.stringify({ ...payload, ts: new Date().getTime() });
  if (!channel) {
    throw HttpError(SERVICE_UNAVAILABLE_ERR_CODE, MQ_NOT_READY_ERR_MESSAGE);
  }
  channel.sendToQueue(REMOVE_REMINDER_BY_USER_ID_MQ_QUEUE, Buffer.from(msg));
  console.log(' [x] Sent %s', REMOVE_REMINDER_BY_USER_ID_MQ_QUEUE);
  return null;
};
