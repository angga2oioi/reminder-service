...
amqp.connect(process.env.AMQP_HOST, (err, conn) => {
  ...
  conn.createChannel((error1, ch) => {
    ...
    channel = ch;
    channel.assertQueue(PROCESS_REMINDER_MQ_QUEUE, {
      durable: false,
    });
  });
});

exports.submitProcessReminder = (payload = {}) => {
  ...
  channel.sendToQueue(PROCESS_REMINDER_MQ_QUEUE, Buffer.from(msg));
  console.log(' [x] Sent %s', PROCESS_REMINDER_MQ_QUEUE);
  return null;
};
