...
amqp.connect(process.env.AMQP_HOST, (err, conn) => {
  ...
  conn.createChannel((error1, ch) => {
    ...
    channel = ch;
    channel.assertQueue(SEND_REMINDER_MQ_QUEUE, {
      durable: false,
    });
  });
});

exports.submitSendReminder = (payload = {}) => {
  ...
  channel.sendToQueue(SEND_REMINDER_MQ_QUEUE, Buffer.from(msg));
  console.log(' [x] Sent %s', SEND_REMINDER_MQ_QUEUE);
  return null;
};
